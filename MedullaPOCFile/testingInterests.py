...
for camp_idx, camp_row in camp_df.iterrows():
    ...
    if prediction_id:
        ...
        if status == 1:
            ...
            # Fetch debug info for failed prediction (status != 1)
            try:
                pred_debug = ReachFrequencyPrediction(prediction_id).api_get(fields=[ReachFrequencyPrediction.Field.status, ReachFrequencyPrediction.Field.errors, ReachFrequencyPrediction.Field.validation_details])
                print("Prediction debug info:", pred_debug)
            except Exception as e:
                print("Could not fetch prediction debug info:", e)
        ...
        # Remove detailed targeting exclusions (not allowed in ad set creation)
        ...
        for ad_idx, ad_row in adset_ads.iterrows():
            ...
            # If adset creation failed, skip ad creation
            if not adset_id:
                print("Adset creation failed; skipping ad creation for this adset.")
                ad_id = False
