...
# UPDATED: Custom Audience ID for exclusions (replace with your actual custom audience ID)
exclusion_custom_audience_id = "120230031807900477"  # Your saved audience converted to custom audience

# Log what audiences we're working with
print(f"🎯 Custom Audience ID for exclusions: {exclusion_custom_audience_id}")

# Build exclusions structure for current Meta API
def build_exclusions_from_saved_audience():
    """
    Since detailed targeting exclusions are removed, we need to use custom audiences
    and geo exclusions only. Convert your saved audience to a custom audience first.
    """
    exclusions = {}
    excluded_geo = {}
    
    # OPTION 1: Use custom audience exclusions (RECOMMENDED)
    # Convert your saved audience to a custom audience and use its ID here
    excluded_custom_audiences = [{"id": exclusion_custom_audience_id}]
    
    # OPTION 2: Manual geo exclusions (if needed)
    # Add specific regions/cities to exclude based on your saved audience
    excluded_regions = [
        # Add your specific regions here, example:
        # {"key": "4081", "name": "California"}  
    ]
    
    excluded_cities = [
        # Add your specific cities here, example:
        # {"key": "777934", "name": "San Francisco", "radius": 25, "distance_unit": "mile"}
    ]
    
    if excluded_regions or excluded_cities:
        excluded_geo = {
            "regions": excluded_regions,
            "cities": excluded_cities
        }
    
    return excluded_custom_audiences, excluded_geo
...
# Get exclusions
excluded_custom_audiences, excluded_geo = build_exclusions_from_saved_audience()
...
for camp_idx, camp_row in camp_df.iterrows():
    ...
    for adset_idx, adset_row in campaign_adsets.iterrows():
        ...
        # Build targeting spec with ONLY supported exclusions
        target_spec = {
        ...
        # Add exclusions - ONLY these work in current Meta API
        if excluded_custom_audiences:
            target_spec["excluded_custom_audiences"] = excluded_custom_audiences
            print(f"🚫 Applied custom audience exclusions: {len(excluded_custom_audiences)} audience(s)")

        if excluded_geo:
            if excluded_geo.get("regions") or excluded_geo.get("cities"):
                target_spec["geo_locations"]["excluded_geo_locations"] = excluded_geo
                print(f"🗺️ Applied geo exclusions: {len(excluded_geo.get('regions', []))} regions, {len(excluded_geo.get('cities', []))} cities")
        ...
        # Create AdSet with the same targeting
        adset_params = {
            ...
            AdSet.Field.targeting: target_spec,  # Same targeting as prediction
        ...
        try:
            ...
            if targeting:
                ...
                # Check excluded custom audiences
                if targeting.get('excluded_custom_audiences'):
                    excluded_ca = targeting['excluded_custom_audiences']
                    print(f"   🚫 Excluded Custom Audiences: {excluded_ca}")
                else:
                    print(f"   🚫 Excluded Custom Audiences: None found")
                
                # Check excluded geo locations
                if targeting.get('geo_locations', {}).get('excluded_geo_locations'):
                    excluded_geo_actual = targeting['geo_locations']['excluded_geo_locations']
                    print(f"   🚫 Excluded Geo Locations: {excluded_geo_actual}")
                else:
...
print("="*80)
print(f"📊 Results saved to: finaloutput.csv")
if excluded_custom_audiences:
    print(f"🚫 Exclusions applied via Custom Audiences: {len(excluded_custom_audiences)} audience(s)")
else:
    print("⚠️  No exclusions applied - update exclusion_custom_audience_id with valid custom audience")
print("="*80)
